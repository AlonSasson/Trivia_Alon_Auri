...
def main():
    ...
    for i in range(len(questions)): # get only important data about the question
        ...
        question['answers'] = {str(i): question['answers'][i] for i in range(len(question['answers']))} # convert list to dict
        question['answers'] = html.unescape(str(question['answers']).replace('\'', '"'))
